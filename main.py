...
def main():
    ...
    global partitions
    liste = Listbox(window, height=20, width=28)
    partitions = []
    ...
    btn_markov = Checkbutton(window, text="Chaine de Markov\nSélectionez une musique de référence puis le nombre de notes", variable=m)
    ...
    btn_reset_markov = Checkbutton(window, text="Réinitialiser les musiques de référence pour la prochaine chaine de Markov", variable=mr)
    ...
    btn_1 = Button(window, text="Jouer", width=15,
                   command=lambda: play(partitions[get_index()], mn.get(), mr.get(), canvas, k.get(), invert.get(), m.get(), partition_markov=text.get("1.0", "end")))
    btn_1.pack()
