...
def decode_partition(partition, note_to_frequency, quaver):
    ...
    """
    # We get the durations of all figures
    figures_to_durations = get_figures_duration(quaver)

    # We decode every note in the partitions
    decoded_partition = []
    for note in partition.split():
        # If it's a 'p' the duration depend form the previous note
        if note == 'p':
...
def encode_partition(partition, note_to_frequency, quaver):
    ...
    # For every note we find the corresponding symbol
    last_note = (None, 0)
    encoded_partition = []
    for frequency, duration in partition:
        # If it's a half the duration of the previous note it's a 'p'
